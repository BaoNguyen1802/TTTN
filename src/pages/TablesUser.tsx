...
const TablesUser = () => {
  const [userData, setUserData] = useState<any[]>([]); // Khởi tạo state lưu trữ dữ liệu người dùng
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get(env+ '/api/v1/user/all');
  ...
  return (
    <>
      <Breadcrumb pageName="Users Manager" />
            ...
            >
              <div className="col-span-1 flex items-center">
                <p className="text-sm text-black dark:text-white">
                  {user._id ? `${user._id.slice(0, 3)}...${user._id.slice(-4)}` : 'N/A'}
                </p>
              ...
              <div className="col-span-1 flex items-center">
                <div className="h-12 w-12 rounded-full">
                  <img src={user.avatar ? user.avatar : "https://upload.wikimedia.org/wikipedia/commons/9/99/Sample_User_Icon.png"} alt={user.username} className="w-full h-full object-cover rounded-full" />
                </div>
              </div>
              <div className="col-span-1 flex items-center">
